...
void sendMsg(Client *client) {
    ...
    file = fopen(buffer,"a");
    sprintf(msg,"Zadaj spravu pre uzivatela %s",receiver);
    write(client->sockfd,msg, strlen(msg)+1);
    bzero(buffer,MAX_BUFFER_SIZE);
    bzero(msg,MAX_BUFFER_SIZE);
    read(client->sockfd,msg,MAX_BUFFER_SIZE);
    printf("Uzivatel %s posiela spravu %s uzivatelovi %s\n",client->name,msg, receiver);
    ...
    {
        buffer[i] = buffer[i] + 3; //the key for encryption is 3 that is added to ASCII value
    }
...
void makeGroup(Client * client) {
    ...
    if(strcmp(buffer,"1")==0 )
    {
        printf("Uzivatel %s si zvolil ze chce poslat vsetkym clenom skupiny spravu\n", client->name);
        msg="Zadaj spravu, ktoru chces poslat skupine";
        write(client->sockfd,msg, strlen(msg)+1);
        read(client->sockfd,buffer,sizeof(buffer));
        ...
        for(int i = 0; (i < MAX_BUFFER_SIZE && message[i] != '\0'); i++)
        {
            printf("Sifrujem\n");
...
void onlineUserMenu(Client *client) {
    ...
    printf("Uzivatel %s ma %d ziadosti o priatelstvo %d\n",client->name,client->numOfAddRequests);
    if (client->numOfAddRequests > 0) {
        checkNewAddRequests(client);
    }
